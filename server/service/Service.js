...
const getUsers = async () => {
  try {
    const { data, error } = await supabase.from("Users")
      .select(`id, name, email, phone, position_id, 
				Department ( name, id ),
				Position ( name, id )
				`);
...
const getPositions = async () => {
  try {
    const { data, error } = await supabase
      .from("Position")
      .select(`id, name, department_id, total_places, vacant_places`);
...
const updateUser = async (req) => {
  	try {
		const { data: Position, positionError } = await supabase
		.from("Position")
		.select("department_id")
		.eq("id", req.body.position_id);
